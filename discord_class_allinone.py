...
Testimine, mis juhtuks, kui töödelda korraga mitut serverit.
Kahtlane ollus, mis ei tööta.
TODO: Asi tööle saada, seleks on vaja midagi teha.
...
meta:
    channels (dict)
        kanali id
            name
            server
    servers - serveri(te) info Lisaks arvestada serveri indeksiga.
        name
        type
...
kategooriad_java = {"food": {"Yldine", "Kokku"}, "general": {"Yldine", "Kokku"},
                    "kaugōpe": {"Yldine", "Kokku"}, "konsult": {"Yldine", "Kokku"}, "meme": {"Yldine", "Kokku"},
                    "java": {"Yldine", "Kokku"}, "random": {"Yldine", "Kokku"}, "stat": {"Yldine", "Kokku"}}
...
class Stats:
    ...
    def __init__(self, fname='dht_kokku.txt',OUTPUT_FOLDER='Allinone/',  sname='stats.xlsx', kategooria=kategooriad_py):
        """
        ###   INIT1   ###.
    ...
    def __init2(self):
        ...
        for c in sorted(self.archive['data'], key=lambda x:(self.archive['meta']['channels'][x]['server'],x)):  # c = kanali id
            server_id=sts.archive['meta']['channels'][c]['server']
            server_name=self.archive['meta']['servers'][server_id]['name']
            cur_name = self.archive['meta']['channels'][c]['name']  # Praeguse kanali nimi
            old_cur_name = cur_name.split('_')[0]
            cur_name = server_name+'_'+old_cur_name
            prev_msg = -1
            print(cur_name[0]+old_cur_name[:1] + old_cur_name[-2:], end=' ')
            for m in sorted(self.archive['data'][c]):  # m = sõnumi ID
                ...
                for sub in [cur_name.split('_')[1]] + list(self.kategooriad[cur_name.split('_')[1]]):
                    sub = server_name+'_'+sub
...
"""
print('Kõik')
z = {**kategooriad_java, **kategooriad_py, **kategooriad_kaug}
sts = Stats('dht_kokku.txt', 'allinone/', kategooria=z)  # Java
sts._Stats__init2()
sts.times2_cleanup()
sts.ajatabel_suur()
sts.arhiiv()
...
# ani.draw_main()
"""
for ch_id in sts.archive['meta']['channels']:
	print(ch_id, sts.archive['meta']['channels'][ch_id]['name'],
              sts.archive['meta']['servers'][sts.archive['meta']['channels'][ch_id]['server']]['name'], sep='\t')
"""
