...
class ilm:
    ...
    def weatherAPI(lat, lng):
        dld = ['vt1alerts', 'vt1currentdatetime', 'vt1dailyForecast', 'vt1hourlyForecast', 'vt1observation',
               'vt1pollenforecast', 'vt1precipitation', 'vt1wwir']
        vot = 'd522aa97197fd864d36b418f39ebb323'  # 'c1ea9f47f6a88b9acb43aba7faf389d4'
        adr = ''.join(['http://api.weather.com/v2/turbo/', ';'.join(dld), '?units=m&language=et-EE&geocode=',
                       str(round(float(lat), 2)), ',', str(round(float(lng), 2)), '&format=json&apiKey=', vot])
