...
class stats:
    def __init__(self,fname='dht.txt'):
        ...
        # self.nädal=["Esmaspäev", "Teisipäev", "Kolmapäev", "Neljapäev", "Reede", "Laupäev", "Pühapäev"]
        self.nädal=["Esmasp.", "Teisip.", "Kolmap.", "Neljap.", "Reede", "Laup.", "Pühap."]
        self.init2()
    def init2(self):
        ###   INIT2   ###
    ...
    def arhiiv():
        ...
        for x in list(self.users):
            ...
            for kanal in channels:
                if kanal in self.users[x]['count']:
                    ...
                    out3.append(str(round(self.users[x]['lens'][kanal] / self.users[x]['count'][kanal], 3)).replace('.', ','))
                else:
                    out1.append('0')
                    out2.append('0')
                    out3.append('0')
            ...
            out3.append(str(round(self.users[x]['lens']['total'] / self.users[x]['count']['total'], 3)).replace('.', ','))
        ...
        c = 1
        for i in sorted(lyhi, key=lambda x: self.users[x]['count']['total'], reverse=True):
            out = '\t'.join([str(c), self.users[i]['n'], str(lyhi[i]), str(self.users[i]['count']['total']),
                             str(round(lyhi[i] / self.users[i]['count']['total'] * 100, 2)).replace('.', ',')])
            sisukus.append(out)
            c += 1
    ...
    def ajatabelVäiksem(uid, kanal):
        ...
        for i in range(7):
            out=['',nädal[i]]+list(map(str,self.users[uid]['times'][kanal][24*i:24*(i+1)]))
    ...
    def graafid_edetabel(username, uid=0):
        ...
        print()
    class output:
        def users_py(fname='users.py'):
            with open(fname,'w',encoding='utf-8') as f:
                f.write('users='+str(self.users))
        def users_json(fname='ergo.json'):
            use=json.dumps(self.users)
            with open(fname,'w',encoding='utf-8') as f:
                f.write(str(use))
        def tgf_tag(fname='disco_tag.tgf'):
            with open(fname, 'w', encoding='utf-8') as f:
        ...
        def tgf_msg(fname='disco_msg.tgf'):
            with open(fname, 'w', encoding='utf-8') as f:
                for i in filter(lambda x:x>-1,self.users):
                    print(i,self.users[i]['n'],file=f)
                print('#',file=f)
    ...
    def graafik():
        ...
        for i in channels:
            ...
            c+=1
        pygame.init()
        lava = pygame.display.set_mode((min(len(times),10000), len(self.users)), 0, 32)
